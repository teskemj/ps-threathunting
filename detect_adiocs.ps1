# Teske Lab Threat IOC Detection Script (Optimized with FilterHashtable)
# Timestamp: 2025-06-02T16:48
...
Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4662; StartTime=$Now.AddDays(-1)} |
...
Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-PowerShell/Operational'; Id=4104; StartTime=$Now.AddDays(-1)} |
...
Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4624; StartTime=$Now.AddDays(-1)} |
...
Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4698; StartTime=$Now.AddDays(-1)} |
ForEach-Object {
    $Results += [PSCustomObject]@{
        TimeCreated = $_.TimeCreated
...
Get-WinEvent -FilterHashtable @{LogName='System'; Id=7045; StartTime=$Now.AddDays(-1)} |
ForEach-Object {
    $Results += [PSCustomObject]@{
        TimeCreated = $_.TimeCreated
