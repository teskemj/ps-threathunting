...
Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4662; StartTime=$Now.AddDays(-30)} |
...
Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-PowerShell/Operational'; Id=4104; StartTime=$Now.AddDays(-30)} |
...
Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4624; StartTime=$Now.AddDays(-30)} |
...
Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4698; StartTime=$Now.AddDays(-30)} |
ForEach-Object {
    $Results += [PSCustomObject]@{
        TimeCreated = $_.TimeCreated
...
Get-WinEvent -FilterHashtable @{LogName='System'; Id=7045; StartTime=$Now.AddDays(-30)} |
ForEach-Object {
    $Results += [PSCustomObject]@{
        TimeCreated = $_.TimeCreated
